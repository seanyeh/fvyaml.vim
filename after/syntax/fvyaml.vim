...
syn keyword yamlConstant TRUE True true
syn keyword yamlConstant FALSE False false

syn match  yamlKey	"^\s*\zs[^ \t\"\'#]\+\ze\s*:"

" Highlight first value of every object in a list
syn region yamlFirstKey	start="^\s*-\s*[^ \t\"\'#]\+\s*:" end="$" oneline contains=yamlKey,yamlFirstValue
syn match yamlKey           "^\s*-\s*\zs[^ \t\"\'#]\+\ze\s*:" contained
syn match yamlFirstValue    ":\zs.*\ze" contained
